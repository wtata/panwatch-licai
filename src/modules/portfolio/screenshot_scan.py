...
【同花顺 App 常见列（无证券代码）】
从左到右通常是：名称 | 盈亏 | 持仓/可用 | 成本/现价
每个数据列经常上下叠两行：
- 第 2 列盈亏：金额和涨跌幅，整列忽略，不要当成本和数量
- 第 3 列持仓/可用：上面是持仓，下面是可用。quantity 只取上面的持仓，不要用可用
- 第 4 列成本/现价：上面是成本，下面是现价。avgCost 只取上面的成本，不要用现价
- name：左侧中文简称，如浙江鼎业、沪电股份
- A 股持仓通常是 100 的整数倍
- 持仓为 0 的行跳过（App 常把 0 持仓隐藏）。不要编造非零数量，不要用「可用」代替 0 持仓
- 「多多」这类缺字应写成「多氟多」，不要输出残字
- code：App 截图经常没有代码。没有就填空字符串，不要编造
- 港股成本可能带 HK$，只保留数字
...
VISION_USER_PROMPT = (
    "这是持仓截图。按列提取每一只股票的名称、持仓（不要可用）、成本（不要现价）；"
    "整列忽略盈亏。没有代码就留空。不要提取券商名、汇总数字、会议浮窗。只输出 JSON 数组。"
)
